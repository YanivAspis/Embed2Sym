...
class Embed2SymModel:
    ...
    def build_neural_models(self):
        ...
        self.reasoning_embeddings = self._reasoning_network(self.reasoning_input)
        if "output_passthrough" in self._config and self._config["output_passthrough"]:
            self.model_outputs = [
                tf.keras.layers.Softmax(name=target.name.replace('(', "_").replace(')', ""))(embedding)
                for embedding, target in
                zip(self.reasoning_embeddings, self._task_metadata.targets.values())]
        else:
            self.model_outputs = [
                tf.keras.layers.Dense(units=len(self._task_metadata.target_concepts[target.target_concept_name].values),
                                      activation=tf.keras.activations.sigmoid,
                                      name=target.name.replace('(', "_").replace(')', ""))(self.reasoning_embeddings)
    ...
    def train_neural_only(self, dataset_creator, checkpoint_folder):
        ...
        if "learning_rate_schedule" in self._config:
            fit_args["callbacks"].append(
                tf.keras.callbacks.LearningRateScheduler(self._config["learning_rate_schedule"]))

        if checkpoint_folder is not None:
            makedirs(checkpoint_folder, exist_ok=True)
            if "best_model_evaluator" in self._config and self._config["best_model_evaluator"] == "loss":
                max_or_min = "min"
                target_name, metric_name = next(iter(self._config["loss"].items()))
                if len(metric_funcs) == 1:
                    metric_to_monitor = "val_{}".format(metric_name)
                else:
                    metric_to_monitor = "val_{}_{}".format(self.model_outputs[0].name.split('/')[0], "loss")
            else:
    ...
    def evaluate_neural_symbolic_model(self, data_creator, valid_test):
        assert valid_test in ["valid", "test"]
        ds = data_creator.get_full_dataset(valid_test, shuffle=False)
        if "ns_test_batch_size" in self._config:
            ds.batch_size = self._config["ns_test_batch_size"]
        average_accuracy = 0.0
